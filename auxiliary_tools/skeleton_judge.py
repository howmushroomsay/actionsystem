...
def action_evaluation(skeleton_T, skeleton_S, keypoints=None, calibration=True, vis_param=0.8, mov_param=0.001,
    ...
    # step 2
    BODY_PARTS = {"HipCeneter": 0, "LHip": 1, "LKnee": 2, "LAnkle": 3, "RHip": 4,
                  "RKnee": 5, "RAnkle": 6, "LShoulder": 7, "LElbow": 8, "LWrist": 9,
                  "RShoulder": 10, "RElbow": 11, "RWrist": 12, "ShoulderCenter": 13, "Head": 14,
                  }
    if keypoints is not None:
        skeleton_T_kps = skeleton_T[:, keypoints, :]
        skeleton_S_kps = skeleton_S[:, keypoints, :]
    else:
        kps = ["LKnee", "LAnkle", "LHip",  # 增加左髋
               "RKnee", "RAnkle", "RHip",  # 增加右髋
               "LShoulder", "LElbow", "LWrist",
               "RShoulder", "RElbow", "RWrist",
               ]

        keypoints = [BODY_PARTS[kp] for kp in kps]
        skeleton_T_kps = skeleton_T[:, keypoints, :]
        skeleton_S_kps = skeleton_S[:, keypoints, :]
    ...
    # step 4
    warp_param = min(10, int((skeleton_S_filted.shape[0]) / (skeleton_T_filted.shape[0] + 1e-6)))
    warp_param = max(10, warp_param)
    ...
    for kf_idx in range(skeleton_T_filted_len):
        ...
        match_S.append(min_index)
        cost_list.append(cost)

        # if cost < 0.05:
        #     path_valid += 1

    match_S_filtered = list(set(match_S))
    match_S_filtered.sort()
    match_S_filtered_len = len(match_S_filtered)
    match_T_filtered = []
    cost_list_filtered = []
    for kf_idx in range(match_S_filtered_len):
        index = np.where(np.asarray(match_S) == match_S_filtered[kf_idx])[0]
        index_len = len(index)
        cost = 100
        min_index = 0
        for idx in range(index_len):
            p_T = int(match_T[index[idx]])
            p_S = int(match_S[index[idx]])
            if cost_list[index[idx]] < cost:
                cost = cost_list[index[idx]]
                min_index = p_S

        match_T_filtered.append(p_T)
        cost_list_filtered.append(cost)

        if cost < 0.05:
            path_valid += 1
    similarity = path_valid / len(match_T_filtered)
    grade_t = [0.6, 0.72, 0.85]
    grade = sim2grade(similarity, grade_t)

    # step 5
    cost_list = np.asarray(cost_list_filtered)
    ...
    kps_gps = [["LKnee", "LAnkle", "LHip"],  # 左腿
               ...
               ["LShoulder", "LElbow", "LWrist"],  # 左胳膊
               ["RShoulder", "RElbow", "RWrist"],  # 右胳膊
               ]
    gps_idx = {"0": "左腿",
               "1": "右腿",
               "2": "左胳膊",
    ...
    grp = []
    skeleton_T_kps = skeleton_T[disp_T, :, :]
    skeleton_S_kps = skeleton_S[disp_S, :, :]
    for frame_idx in range(len(disp_T)):
        skeleton_T_kp = skeleton_T_kps[frame_idx, :, :]
        skeleton_S_kp = skeleton_S_kps[frame_idx, :, :]

        sim_m = 0
        min_idx = 0
        for group in range(len(kps_gps)):
            keypoints = [BODY_PARTS[kp] for kp in kps_gps[group]]
            pose1 = skeleton_T_kp[keypoints, :]
            pose2 = skeleton_S_kp[keypoints, :]
            sim = cosine_distance(pose1, pose2, keep_dim=False)
            if sim > sim_m:
                sim_m = sim
                min_idx = group
        grp.append(gps_idx[str(min_idx)])
    return similarity, grade, disp_T, disp_S, grp
